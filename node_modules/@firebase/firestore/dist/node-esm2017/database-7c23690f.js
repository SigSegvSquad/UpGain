import { getUA, getModularInstance } from '@firebase/util';
...
const X_GOOG_API_CLIENT_VALUE = `gl-node/${process.versions.node} fire/${version} grpc/${version$1}`;
...
class DocumentReference {
    /** @hideconstructor */
    constructor(firestore, _converter, _key) {
...
class Query {
    ...
    constructor(firestore, _converter, _query) {
        this._converter = _converter;
        this._query = _query;
        /** The type of this Firestore reference. */
        this.type = 'query';
...
class QuerySnapshot {
    ...
    forEach(callback, thisArg) {
        this._snapshot.docs.forEach(doc => {
            callback.call(thisArg, new QueryDocumentSnapshot$1(this._firestore, this._userDataWriter, doc.key, doc, new SnapshotMetadata(this._snapshot.mutatedKeys.has(doc.key), this._snapshot.fromCache), this.query._converter));
        });
    }
    /**
...
function changesFromSnapshot(querySnapshot, includeMetadataChanges) {
    if (querySnapshot._snapshot.oldDocs.isEmpty()) {
        ...
        return querySnapshot._snapshot.docChanges.map(change => {
            const doc = new QueryDocumentSnapshot$1(querySnapshot._firestore, querySnapshot._userDataWriter, change.doc.key, change.doc, new SnapshotMetadata(querySnapshot._snapshot.mutatedKeys.has(change.doc.key), querySnapshot._snapshot.fromCache), querySnapshot.query._converter);
            lastDoc = change.doc;
            return {
                type: 'added',
                doc,
...
class Transaction$1 {
    ...
    get(documentRef) {
        ...
        return this._transaction.lookup([ref._key]).then(docs => {
            ...
            const doc = docs[0];
            if (doc.isFoundDocument()) {
                return new DocumentSnapshot(this._firestore, userDataWriter, doc.key, doc, ref._converter);
            }
            else if (doc.isNoDocument()) {
                return new DocumentSnapshot(this._firestore, userDataWriter, ref._key, null, ref._converter);
            }
            else {
                throw fail();
            }
...
function setDoc(reference, data, options) {
    reference = cast(reference, DocumentReference);
    const firestore = cast(reference.firestore, FirebaseFirestore$1);
    const convertedValue = applyFirestoreDataConverter(reference._converter, data, options);
    const dataReader = newUserDataReader(firestore);
    const parsed = parseSetData(dataReader, 'setDoc', reference._key, convertedValue, reference._converter !== null, options);
...
function addDoc(reference, data) {
    ...
    const convertedValue = applyFirestoreDataConverter(reference._converter, data);
    const dataReader = newUserDataReader(reference.firestore);
    const parsed = parseSetData(dataReader, 'addDoc', docRef._key, convertedValue, reference._converter !== null, {});
...
class Transaction$3 {
    ...
    get(documentRef) {
        ...
        return this._delegate
            .get(ref)
            .then(result => new DocumentSnapshot$2(this._firestore, new DocumentSnapshot$1(this._firestore._delegate, this._userDataWriter, result._key, result._document, result.metadata, ref._converter)));
    }
    set(documentRef, data, options) {
        const ref = castReference(documentRef);
        if (options) {
...
class DocumentReference$1 {
    ...
    onSnapshot(...args) {
        ...
        const observer = wrapObserver(args, result => new DocumentSnapshot$2(this.firestore, new DocumentSnapshot$1(this.firestore._delegate, this._userDataWriter, result._key, result._document, result.metadata, this._delegate._converter)));
    ...
    get(options) {
        ...
        return snap.then(result => new DocumentSnapshot$2(this.firestore, new DocumentSnapshot$1(this.firestore._delegate, this._userDataWriter, result._key, result._document, result.metadata, this._delegate._converter)));
    }
    withConverter(converter) {
...
//# sourceMappingURL=database-7c23690f.js.map
