import { _getProvider, getApp, _removeServiceInstance, _registerComponent, registerVersion } from '@firebase/app';
...
function logDebug(msg, ...obj) {
    if (logClient.logLevel <= LogLevel.DEBUG) {
        ...
        logClient.debug(`Firestore (${version$1}): ${msg}`, ...args);
    }
}
function logError(msg, ...obj) {
    if (logClient.logLevel <= LogLevel.ERROR) {
        const args = obj.map(argToString);
        logClient.error(`Firestore (${version$1}): ${msg}`, ...args);
    }
}
function logWarn(msg, ...obj) {
    if (logClient.logLevel <= LogLevel.WARN) {
        const args = obj.map(argToString);
        logClient.warn(`Firestore (${version$1}): ${msg}`, ...args);
    }
}
/**
...
function fail(failure = 'Unexpected state') {
    ...
    const message = `FIRESTORE (${version$1}) INTERNAL ASSERTION FAILED: ` + failure;
    logError(message);
...
const X_GOOG_API_CLIENT_VALUE = `gl-node/${process.versions.node} fire/${version$1} grpc/${version$2}`;
